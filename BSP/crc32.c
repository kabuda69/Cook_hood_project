...
//CRC32校验表（标准CRC32多项式：0xEDB88320）
//与python的zlib.crc32函数计算结果一致
...
//CRC32计算函数，返回计算得到的CRC32值
u32 CRC32_Calculate(u8 *data, u32 length)
{
    u32 crc = 0xFFFFFFFF;//初始值
    u32 i;//循环变量
    for (i = 0; i < length; i++)//每个字节
    {
        //计算CRC32值
        crc = (crc >> 8) ^ crc32_table[(crc ^ (u8)data[i]) & 0xFF];
    }
    return crc ^ 0xFFFFFFFF;//返回最终的CRC32值    
}

//CRC32校验固件数据函数，返回1表示校验通过，0表示校验失败
u8 CRC32_verrifyFirmware(u8 *data,u32 totalLength)
{
 u32 firmwareLen;
    u32 calculatedCRC;
    u32 receivedCRC;
